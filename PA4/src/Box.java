...
//      Box class
//****************************************************************************
// Creation of box from ellipsoid
//
	...
	private void fillMesh()
	{
		...
		for(i=0,theta=-(float)Math.PI;i<m;++i,theta += d_theta)
	    {
			...
			for(j=0,phi=(float)(-0.5*Math.PI);j<n;++j,phi += d_phi)
			{
				...
				mesh.v[i][j].x =center.x +rx* Math.signum(c_phi) * ((float) Math.pow(absU, 0.1))*Math.signum(c_theta)*((float) Math.pow(absV, 2));
				mesh.v[i][j].y =center.y +ry* Math.signum(c_phi) * ((float) Math.pow(absU, 0.1))*Math.signum(s_theta)*((float) Math.pow(absSV, 2));
