/**
 * Body.java - a model for body, head, eyes of the insect
 */
...
/**
 * A model for the body, eyes, and head of the insect as a sphere scaled in one direction.
 * 
...
public class Body extends Circular implements Displayable {
  ...
  public void initialize(final GL2 gl) {
    ...
    //create the head
    gl.glPushMatrix();
    gl.glTranslated(1, 0, this.radius());
    this.glut().glutSolidSphere(.4, 36, 18);
    gl.glPopMatrix();
    
    //create the other eye
    gl.glPushMatrix();
    gl.glTranslated(1.3, 0, this.radius() + 0.25);
    ...
    // the origin
    // create an ellipsoid for the body by scaling a sphere
    gl.glTranslated(0, 0, this.radius());
    gl.glScalef(1.9f, 1f, 1);
